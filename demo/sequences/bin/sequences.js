...
com_github_mathieuanthoine_gaf_Main.prototype = {
	onConverted: function(pEvent) {
		this._gafMovieClip = new com_github_haxePixiGAF_display_GAFMovieClip((js_Boot.__cast(pEvent.target , com_github_haxePixiGAF_core_ZipToGAFAssetConverter)).get_gafBundle().getGAFTimeline("RedRobot"));
		this.setSequence("stand");
		this.stage.addChild(this._gafMovieClip);
		this._gafMovieClip.interactive = true;
		this._gafMovieClip.on("click",$bind(this,this.onClick));
		window.requestAnimationFrame($bind(this,this.gameLoop));
	}
	,setSequence: function(sequence) {
		this._gafMovieClip.setSequence(sequence);
		(js_Boot.__cast(this._gafMovieClip.getField("sequence") , com_github_haxePixiGAF_display_GAFTextField)).set_text(sequence);
	}
	,onClick: function(pEvent) {
		if(this._gafMovieClip.get_currentSequence() == "walk") {
			this.setSequence("stand");
		} else {
