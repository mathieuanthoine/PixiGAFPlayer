...
com_github_haxePixiGAF_core_ZipToGAFAssetConverter.prototype = $extend(PIXI.utils.EventEmitter.prototype,{
	...
	,findAllAtlasURLs: function() {
		...
		var id = this._gafAssetConfigs.keys();
		while(id.hasNext()) {
			var id1 = id.next();
			var _this = this._gafAssetConfigs;
			gafTimelineConfigs = (__map_reserved[id1] != null ? _this.getReserved(id1) : _this.h[id1]).get_timelines();
			var _g = 0;
			while(_g < gafTimelineConfigs.length) {
				var config = gafTimelineConfigs[_g];
				++_g;
				var folderURL = com_github_haxePixiGAF_core_ZipToGAFAssetConverter.getFolderURL(id1);
				var _g1 = 0;
		...
		if(this._atlasSourceURLs.length > 0) {
			...
			while(_g11 < _g7) {
				var _atlasSourceIndex = _g11++;
				url1 = this._atlasSourceURLs[_atlasSourceIndex];
				fileName = url1.substring(url1.lastIndexOf("/") + 1);
				var this1 = this._taGFXs;
	...
	,createGAFTimelines: function(event) {
		...
		while(_g1 < _g) {
			var i1 = _g1++;
			gafAssetConfigID = this._gafAssetsIDs[i1];
			var _this = this._gafAssetConfigs;
			if(__map_reserved[gafAssetConfigID] != null) {
				gafAssetConfig = _this.getReserved(gafAssetConfigID);
			} else {
				gafAssetConfig = _this.h[gafAssetConfigID];
			}
	...
	,createTimeline: function(config,asset) {
		...
		var _g6 = com_github_haxePixiGAF_core_ZipToGAFAssetConverter.actionWithAtlases;
		switch(_g6) {
	...
	,onConverted: function(event) {
		...
		var this1 = this._gafAssetConfigs;
...
com_github_haxePixiGAF_data_GAFAsset.prototype = {
	...
	,addGAFTimeline: function(timeline) {
		var this1 = this._timelinesDictionary;
		var key = timeline.get_id();
		var _this = this1;
		if((__map_reserved[key] != null ? _this.getReserved(key) : _this.h[key]) == null) {
			var this2 = this._timelinesDictionary;
			...
			if(timeline.get_config().get_linkage() != null) {
				var this3 = this._timelinesByLinkage;
	...
	,getGAFTimelineByLinkage: function(linkage) {
		...
		var gafTimeline = __map_reserved[linkage] != null ? _this.getReserved(linkage) : _this.h[linkage];
		return gafTimeline;
	}
			...
			if(atlasScale.get_scale() == scale) {
				...
				while(_g3 < _g2) {
					...
					if(atlasCSF.get_csf() == csf) {
						...
						if(element != null) {
							var texture = atlasCSF.get_atlas().getTextureByIDAndAtlasID(element.get_id(),element.get_atlasID());
...
com_github_haxePixiGAF_data_GAFBundle.prototype = {
	...
	,addGAFAsset: function(gafAsset) {
		var this1 = this._gafAssetsDictionary;
		var key = gafAsset.get_id();
		var _this = this1;
		if((__map_reserved[key] != null ? _this.getReserved(key) : _this.h[key]) == null) {
			var this2 = this._gafAssetsDictionary;
...
com_github_haxePixiGAF_data_GAFGFXData.prototype = $extend(PIXI.utils.EventEmitter.prototype,{
	addTAGFX: function(scale,csf,imageID,taGFX) {
		...
		if((__map_reserved[lScale] != null ? _this.getReserved(lScale) : _this.h[lScale]) == null) {
			var this1 = this._taGFXDictionary;
		...
		var _this3 = __map_reserved[lScale] != null ? _this2.getReserved(lScale) : _this2.h[lScale];
		if((__map_reserved[lCsf] != null ? _this3.getReserved(lCsf) : _this3.h[lCsf]) == null) {
			var _this4 = this._taGFXDictionary;
			var this2 = __map_reserved[lScale] != null ? _this4.getReserved(lScale) : _this4.h[lScale];
			var v1 = new haxe_ds_StringMap();
			var _this5 = this2;
			if(__map_reserved[lCsf] != null) {
				_this5.setReserved(lCsf,v1);
			} else {
				_this5.h[lCsf] = v1;
			}
		}
		var _this6 = this._taGFXDictionary;
		var _this7 = __map_reserved[lScale] != null ? _this6.getReserved(lScale) : _this6.h[lScale];
		var _this8 = __map_reserved[lCsf] != null ? _this7.getReserved(lCsf) : _this7.h[lCsf];
		if((__map_reserved[imageID] != null ? _this8.getReserved(imageID) : _this8.h[imageID]) == null) {
			var _this9 = this._taGFXDictionary;
			var _this10 = __map_reserved[lScale] != null ? _this9.getReserved(lScale) : _this9.h[lScale];
			var _this11 = __map_reserved[lCsf] != null ? _this10.getReserved(lCsf) : _this10.h[lCsf];
			if(__map_reserved[imageID] != null) {
				_this11.setReserved(imageID,taGFX);
			} else {
				_this11.h[imageID] = taGFX;
			}
		}
	}
	,getTAGFXs: function(scale,csf) {
		...
		if(this._taGFXDictionary != null) {
			...
			if((__map_reserved[lScale] != null ? _this.getReserved(lScale) : _this.h[lScale]) != null) {
				...
				var _this2 = __map_reserved[lScale] != null ? _this1.getReserved(lScale) : _this1.h[lScale];
				if(__map_reserved[lCsf] != null) {
					return _this2.getReserved(lCsf);
				} else {
					return _this2.h[lCsf];
				}
			}
		}
		return null;
	...
	,getTAGFX: function(scale,csf,imageID) {
		...
		if(this._taGFXDictionary != null) {
			...
			if((__map_reserved[lScale] != null ? _this.getReserved(lScale) : _this.h[lScale]) != null) {
				var _this1 = this._taGFXDictionary;
				var _this2 = __map_reserved[lScale] != null ? _this1.getReserved(lScale) : _this1.h[lScale];
				if((__map_reserved[lCsf] != null ? _this2.getReserved(lCsf) : _this2.h[lCsf]) != null) {
					var _this3 = this._taGFXDictionary;
					var _this4 = __map_reserved[lScale] != null ? _this3.getReserved(lScale) : _this3.h[lScale];
					var _this5 = __map_reserved[lCsf] != null ? _this4.getReserved(lCsf) : _this4.h[lCsf];
					if(__map_reserved[imageID] != null) {
						return _this5.getReserved(imageID);
					} else {
						return _this5.h[imageID];
					}
				}
			}
		}
	...
	,createTextures: function(scale,csf) {
		...
		if(taGFXs != null) {
			...
			if((__map_reserved[lScale] != null ? _this.getReserved(lScale) : _this.h[lScale]) == null) {
				var this1 = this._texturesDictionary;
				var v = new haxe_ds_StringMap();
				var _this1 = this1;
				if(__map_reserved[lScale] != null) {
					_this1.setReserved(lScale,v);
			...
			var imageAtlasID = taGFXs.keys();
			while(imageAtlasID.hasNext()) {
				var imageAtlasID1 = imageAtlasID.next();
				if((__map_reserved[imageAtlasID1] != null ? taGFXs.getReserved(imageAtlasID1) : taGFXs.h[imageAtlasID1]) != null) {
					var _this6 = this._texturesDictionary;
					var _this7 = __map_reserved[lScale] != null ? _this6.getReserved(lScale) : _this6.h[lScale];
					this.addTexture(__map_reserved[lCsf] != null ? _this7.getReserved(lCsf) : _this7.h[lCsf],__map_reserved[imageAtlasID1] != null ? taGFXs.getReserved(imageAtlasID1) : taGFXs.h[imageAtlasID1],imageAtlasID1);
				}
			}
			return true;
		}
	...
	,createTexture: function(scale,csf,imageID) {
		...
		if(taGFX != null) {
			...
			var _this6 = this._texturesDictionary;
			var _this7 = __map_reserved[lScale] != null ? _this6.getReserved(lScale) : _this6.h[lScale];
			this.addTexture(__map_reserved[lCsf] != null ? _this7.getReserved(lCsf) : _this7.h[lCsf],taGFX,imageID);
			return true;
		}
		return false;
	}
	,getTexture: function(scale,csf,imageID) {
		...
		if(this._texturesDictionary != null) {
			...
			if((__map_reserved[lScale] != null ? _this.getReserved(lScale) : _this.h[lScale]) != null) {
				...
				if((__map_reserved[lCsf] != null ? _this2.getReserved(lCsf) : _this2.h[lCsf]) != null) {
					...
					var _this4 = __map_reserved[lScale] != null ? _this3.getReserved(lScale) : _this3.h[lScale];
					var _this5 = __map_reserved[lCsf] != null ? _this4.getReserved(lCsf) : _this4.h[lCsf];
					if((__map_reserved[imageID] != null ? _this5.getReserved(imageID) : _this5.h[imageID]) != null) {
						var _this6 = this._texturesDictionary;
						var _this7 = __map_reserved[lScale] != null ? _this6.getReserved(lScale) : _this6.h[lScale];
						var _this8 = __map_reserved[lCsf] != null ? _this7.getReserved(lCsf) : _this7.h[lCsf];
						if(__map_reserved[imageID] != null) {
							return _this8.getReserved(imageID);
						} else {
							return _this8.h[imageID];
						}
					}
				}
			}
		...
		if(this.createTexture(scale,csf,imageID)) {
			var _this9 = this._texturesDictionary;
			var _this10 = __map_reserved[lScale] != null ? _this9.getReserved(lScale) : _this9.h[lScale];
			var _this11 = __map_reserved[lCsf] != null ? _this10.getReserved(lCsf) : _this10.h[lCsf];
			if(__map_reserved[imageID] != null) {
				return _this11.getReserved(imageID);
			} else {
				return _this11.h[imageID];
			}
		}
		return null;
	}
	...
	,get_isTexturesReady: function() {
		var empty = true;
		var tagfx = this._textureLoadersSet.iterator();
		while(tagfx.hasNext()) {
			var tagfx1 = tagfx.next();
			empty = false;
			break;
		}
...
com_github_haxePixiGAF_data_GAFTimelineConfig.prototype = {
	...
	,getNamedPartID: function(name) {
		...
		var id = new haxe_ds__$StringMap_StringMapIterator(_this,_this.arrayKeys());
		while(id.hasNext()) {
			var id1 = id.next();
			var _this1 = this._namedParts;
			if((__map_reserved[id1] != null ? _this1.getReserved(id1) : _this1.h[id1]) == name) {
				return id1;
			}
		}
		return null;
...
com_github_haxePixiGAF_data_config_CAnimationFrame.prototype = {
	...
	,addInstance: function(instance) {
		var this1 = this._instancesDictionary;
		var key = instance.get_id();
		var _this = this1;
		if((__map_reserved[key] != null ? _this.getReserved(key) : _this.h[key]) != null) {
			if(instance.get_alpha() != null) {
				var tmp = this._instances;
				var tmp1 = this._instances;
				var this2 = this._instancesDictionary;
				var key1 = instance.get_id();
				var _this1 = this2;
				tmp[tmp1.indexOf(__map_reserved[key1] != null ? _this1.getReserved(key1) : _this1.h[key1])] = instance;
				var this3 = this._instancesDictionary;
				...
				var index = this._instances;
				var this4 = this._instancesDictionary;
				var key2 = instance.get_id();
				var _this3 = this4;
				var index1 = index.indexOf(__map_reserved[key2] != null ? _this3.getReserved(key2) : _this3.h[key2]);
				if(index1 == this._instances.length - 1) {
					this._instances.pop();
				} else {
					this._instances[index1] = this._instances.pop();
				}
				this._instancesDictionary.remove(instance.get_id());
			}
		} else {
			this._instances.push(instance);
			var this5 = this._instancesDictionary;
...
com_github_haxePixiGAF_data_config_CAnimationObjects.prototype = {
	addAnimationObject: function(animationObject) {
		var this1 = this._animationObjectsDictionary;
		var key = animationObject.get_instanceID();
		var _this = this1;
		if((__map_reserved[key] != null ? _this.getReserved(key) : _this.h[key]) == null) {
			var this2 = this._animationObjectsDictionary;
	...
	,getAnimationObject: function(instanceID) {
		...
		if((__map_reserved[instanceID] != null ? _this.getReserved(instanceID) : _this.h[instanceID]) != null) {
			...
			if(__map_reserved[instanceID] != null) {
				return _this1.getReserved(instanceID);
			} else {
				return _this1.h[instanceID];
			}
		} else {
			return null;
		}
...
com_github_haxePixiGAF_data_config_CFilter.prototype = {
	clone: function() {
		...
		while(_g < _g1.length) {
			...
			++_g;
			result.get_filterConfigs().push(filterData.clone());
		}
		return result;
	}
...
com_github_haxePixiGAF_data_config_CTextFieldObjects.prototype = {
	addTextFieldObject: function(textFieldObject) {
		var this1 = this._textFieldObjectsDictionary;
		var key = textFieldObject.get_id();
		var _this = this1;
		if((__map_reserved[key] != null ? _this.getReserved(key) : _this.h[key]) == null) {
			var this2 = this._textFieldObjectsDictionary;
...
com_github_haxePixiGAF_data_config_CTextureAtlas.prototype = {
	dispose: function() {
		...
		var textureAtlas = new haxe_ds__$StringMap_StringMapIterator(_this,_this.arrayKeys());
		while(textureAtlas.hasNext()) {
			var textureAtlas1 = textureAtlas.next();
	...
	,getTexture: function(id) {
		...
		if(textureAtlasElement != null) {
			...
			var pivotMatrix;
			if(this._textureAtlasConfig.get_elements().getElement(id) != null) {
				pivotMatrix = this._textureAtlasConfig.get_elements().getElement(id).get_pivotMatrix();
			} else {
				pivotMatrix = new PIXI.Matrix();
			}
...
com_github_haxePixiGAF_data_config_CTextureAtlasElements.prototype = {
	addElement: function(element) {
		var this1 = this._elementsDictionary;
		var key = element.get_id();
		var _this = this1;
		if((__map_reserved[key] != null ? _this.getReserved(key) : _this.h[key]) == null) {
			var this2 = this._elementsDictionary;
			...
			if(element.get_linkage() != null) {
				var this3 = this._elementsByLinkage;
	...
	,getElementByLinkage: function(linkage) {
		...
		if((__map_reserved[linkage] != null ? _this.getReserved(linkage) : _this.h[linkage]) != null) {
			...
			if(__map_reserved[linkage] != null) {
				return _this1.getReserved(linkage);
			} else {
				return _this1.h[linkage];
			}
		} else {
			return null;
		}
...
com_github_haxePixiGAF_data_converters_BinGAFAssetConfigConverter.readNamedParts = function(tagContent,timelineConfig) {
	...
	while(_g1 < _g) {
		...
		var this1 = timelineConfig.get_namedParts();
		var v = tagContent.readUTF();
		var _this = this1;
		var key = partID == null ? "null" : "" + partID;
...
com_github_haxePixiGAF_data_converters_BinGAFAssetConfigConverter.readTextFields = function(tagContent,timelineConfig) {
	...
	while(_g1 < _g) {
		...
		var hasRestrict = tagContent.readBoolean();
		if(hasRestrict) {
		...
		var blockIndent = _$UInt_UInt_$Impl_$.toFloat(tagContent.readUnsignedInt());
		var bold = tagContent.readBoolean();
		var bullet = tagContent.readBoolean();
		var color = tagContent.readUnsignedInt();
		var font = tagContent.readUTF();
		var indent = tagContent.readUnsignedInt();
		var italic = tagContent.readBoolean();
		var kerning = tagContent.readBoolean();
		var leading = tagContent.readUnsignedInt();
		var leftMargin = _$UInt_UInt_$Impl_$.toFloat(tagContent.readUnsignedInt());
		var letterSpacing = tagContent.readFloat();
		var rightMargin = _$UInt_UInt_$Impl_$.toFloat(tagContent.readUnsignedInt());
		...
		var target = tagContent.readUTF();
		var underline = tagContent.readBoolean();
		var url = tagContent.readUTF();
...
com_github_haxePixiGAF_data_converters_BinGAFAssetConfigConverter.readDropShadowFilter = function(source,filter) {
	...
	var blurX = source.readFloat();
	var blurY = source.readFloat();
	var angle = source.readFloat();
	var distance = source.readFloat();
	var strength = source.readFloat();
...
com_github_haxePixiGAF_data_converters_BinGAFAssetConfigConverter.readColorValue = function(source) {
	...
	var alpha = ((argbValue >> 24 & 255) * 100 / 255 | 0) / 100;
	var color = argbValue & 16777215;
	return [alpha,color];
};
...
com_github_haxePixiGAF_data_converters_BinGAFAssetConfigConverter.prototype = $extend(PIXI.utils.EventEmitter.prototype,{
	...
	,parseStart: function() {
		...
		var _g = this._config.get_compression();
		if(_g == 4669763) {
		...
		if(this._config.get_versionMajor() < 4) {
			...
			this._currentTimeline.set_bounds(new PIXI.Rectangle(this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat()));
			this._currentTimeline.set_pivot(new PIXI.Point(this._bytes.readFloat(),this._bytes.readFloat()));
			...
			while(_g2 < _g1) {
				var i1 = _g2++;
				this._config.get_scaleValues().push(this._bytes.readFloat());
			}
			...
			while(_g21 < _g11) {
				var i2 = _g21++;
				this._config.get_csfValues().push(this._bytes.readFloat());
			}
		}
		this.readNextTag();
	}
	,checkForMissedRegions: function(timelineConfig) {
		if(timelineConfig.get_textureAtlas() != null && timelineConfig.get_textureAtlas().get_contentScaleFactor().get_elements() != null) {
			var _this = timelineConfig.get_animationObjects().get_animationObjectsDictionary();
			var ao = new haxe_ds__$StringMap_StringMapIterator(_this,_this.arrayKeys());
			while(ao.hasNext()) {
				var ao1 = ao.next();
				if(ao1.get_type() == "texture" && timelineConfig.get_textureAtlas().get_contentScaleFactor().get_elements().getElement(ao1.get_regionID()) == null) {
	...
	,readTimeline: function() {
		...
		timelineConfig.set_bounds(new PIXI.Rectangle(this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat()));
		timelineConfig.set_pivot(new PIXI.Point(this._bytes.readFloat(),this._bytes.readFloat()));
		var hasLinkage = this._bytes.readBoolean();
		if(hasLinkage) {
	...
	,readTextureAtlasConfig: function(tagID) {
		var i = 0;
		var j = 0;
		var scale = this._bytes.readFloat();
		...
		var linkageName = "";
		var _g11 = 0;
		var _g4 = elementsLength;
		while(_g11 < _g4) {
			var i2 = _g11++;
			pivot = new PIXI.Point(this._bytes.readFloat(),this._bytes.readFloat());
			topLeft = new PIXI.Point(this._bytes.readFloat(),this._bytes.readFloat());
			if(tagID == 1 || tagID == 8) {
			...
			if(tagID == 8 || tagID == 15) {
				...
				if(hasScale9Grid) {
					scale9Grid = new PIXI.Rectangle(this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat());
				} else {
					scale9Grid = null;
				}
			}
			...
			while(_g1 < _g) {
				...
				if(prevFrame != null) {
					...
					while(missedFrameNumber < currentFrame.get_frameNumber()) {
						timelineConfig.get_animationConfigFrames().addFrame(prevFrame.clone(missedFrameNumber));
						++missedFrameNumber;
					}
				} else {
				...
				if(hasChangesInDisplayList) {
					...
					while(_g3 < _g2) {
						...
						matrix = new PIXI.Matrix(this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat());
						filter = null;
						if(hasColorTransform) {
							var params = [this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat(),this._bytes.readFloat()];
							if(filter == null) {
		...
		while(_g < _g1.length) {
			...
			while(_g2 < _g3.length) {
				...
				while(_g4 < _g5.length) {
					...
					++_g4;
					var animationObject = timeline.get_animationObjects().getAnimationObject(frameInstance.get_id());
					if(animationObject.get_mask()) {
						...
						if(animationObject.get_type() == "texture") {
							...
							var this1 = timeline.get_textFields().get_textFieldObjectsDictionary();
							var key = animationObject.get_regionID();
							var _this = this1;
							var textField = __map_reserved[key] != null ? _this.getReserved(key) : _this.h[key];
	...
	,endParsing: function() {
		...
		var itemIndex = 0;
		if(isNaN(this._config.get_defaultScale())) {
			if(!isNaN(this._defaultScale)) {
		...
		if(isNaN(this._config.get_defaultContentScaleFactor())) {
			itemIndex = 0;
			if(!isNaN(this._defaultContentScaleFactor)) {
...
com_github_haxePixiGAF_data_textures_TextureAtlas.prototype = {
	...
	,getNames: function(prefix,out) {
		...
		if(this._subTextureNames == null) {
			...
			var name1 = this._subTextures.keys();
			while(name1.hasNext()) {
				var name2 = name1.next();
				this._subTextureNames[this._subTextureNames.length] = name2;
			}
...
com_github_haxePixiGAF_display_GAFMovieClip.prototype = $extend(com_github_haxePixiGAF_display_GAFContainer.prototype,{
	...
	,showMaskByID: function(id) {
		...
		if(maskObject != null && stencilMaskObject != null) {
			...
			haxe_Log.trace("WARNING:mask object is missing. It might be disposed.",{ fileName : "GAFMovieClip.hx", lineNumber : 193, className : "com.github.haxePixiGAF.display.GAFMovieClip", methodName : "showMaskByID"});
		}
	}
	,hideMaskByID: function(id) {
		...
		if(stencilMaskObject != null) {
			...
			haxe_Log.trace("WARNING:mask object is missing. It might be disposed.",{ fileName : "GAFMovieClip.hx", lineNumber : 219, className : "com.github.haxePixiGAF.display.GAFMovieClip", methodName : "hideMaskByID"});
		}
	}
	...
	,get_currentSequence: function() {
		var sequence = this._config.get_animationSequences().getSequenceByFrame(this.get_currentFrame());
		if(sequence != null) {
	...
	,advanceTime: function(passedTime) {
		...
		if(this._mcVector != null) {
			var _g11 = 0;
			var _g2 = this._mcVector.length;
			while(_g11 < _g2) {
				var i1 = _g11++;
				this._mcVector[i1].advanceTime(passedTime);
			}
		}
	...
	,showBounds: function(value) {
		if(this._config.get_bounds() != null) {
			haxe_Log.trace("TODO showBounds",{ fileName : "GAFMovieClip.hx", lineNumber : 431, className : "com.github.haxePixiGAF.display.GAFMovieClip", methodName : "showBounds"});
		}
	}
	,copy: function() {
	...
	,_play: function(applyToAllChildren,calledByUser) {
		...
		if(applyToAllChildren && this._config.get_animationConfigFrames().get_frames().length > 0) {
			...
			if(frameConfig.get_actions() != null) {
				...
				while(_g1 < _g) {
					var i1 = _g1++;
					action = frameConfig.get_actions()[i1];
			...
			while(_g11 < _g2) {
				var i2 = _g11++;
				child = js_Boot.__cast(this.getChildAt(i2) , PIXI.Container);
	...
	,_stop: function(applyToAllChildren,calledByUser) {
		...
		if(applyToAllChildren && this._config.get_animationConfigFrames().get_frames().length > 0) {
			...
			while(_g1 < _g) {
				var i = _g1++;
				child = js_Boot.__cast(this.getChildAt(i) , PIXI.Container);
				if(js_Boot.__instanceof(child,com_github_haxePixiGAF_display_GAFMovieClip)) {
					childMC = js_Boot.__cast(child , com_github_haxePixiGAF_display_GAFMovieClip);
					if(calledByUser) {
						childMC.stop(true);
	...
	,checkPlaybackEvents: function() {
		...
		if(com_github_haxePixiGAF_utils_EventEmitterUtility.hasEventListener(this,"typeSequenceStart")) {
			sequence = this._config.get_animationSequences().getSequenceStart(this._currentFrame + 1);
			if(sequence != null) {
		...
		if(com_github_haxePixiGAF_utils_EventEmitterUtility.hasEventListener(this,"typeSequenceEnd")) {
			sequence = this._config.get_animationSequences().getSequenceEnd(this._currentFrame + 1);
			if(sequence != null) {
	...
	,draw: function() {
		...
		if(this._config.get_debugRegions() != null) {
			...
			while(_g1 < _g) {
				var i1 = _g1++;
				this._displayObjectsVector[i1].alpha = 0;
			}
			...
			while(_g11 < _g2) {
				var i2 = _g11++;
				this._mcVector[i2]._hidden = true;
			}
		}
		...
		if(frames.length > this._currentFrame) {
			...
			while(i < l) {
				...
				var this1 = this._displayObjectsDictionary;
				var key = instance.get_id();
				var _this = this1;
				if(__map_reserved[key] != null) {
				...
				if(displayObject != null) {
					...
					if(!(__map_reserved[key1] != null ? animationObjectsDictionary.getReserved(key1) : animationObjectsDictionary.h[key1]).get_mask()) {
						if(instance.get_maskID() != "") {
							...
							var this2 = this._stencilMasksDictionary;
						...
						if(com_github_haxePixiGAF_utils_DebugUtility.RENDERING_DEBUG && js_Boot.__instanceof(displayObject,com_github_haxePixiGAF_display_IGAFDebug)) {
							var colors = com_github_haxePixiGAF_utils_DebugUtility.getRenderingDifficultyColor(instance,this._alphaLessMax,this._masked,this._hasFilter);
							(js_Boot.__cast(displayObject , com_github_haxePixiGAF_display_IGAFDebug)).set_debugColors(colors);
						}
					} else {
						var this3 = this._displayObjectsDictionary;
	...
	,initialize: function(textureAtlas,gafAsset) {
		...
		var animationObjectConfig = new haxe_ds__$StringMap_StringMapIterator(animationObjectsDictionary,animationObjectsDictionary.arrayKeys());
		while(animationObjectConfig.hasNext()) {
			var animationObjectConfig1 = animationObjectConfig.next();
			var _g = animationObjectConfig1.get_type();
			switch(_g) {
			case "textField":
				var tfObj = this._config.get_textFields().get_textFieldObjectsDictionary().get(animationObjectConfig1.get_regionID());
				displayObject = new com_github_haxePixiGAF_display_GAFTextField(tfObj,this._scale,this._contentScaleFactor);
				break;
			case "texture":
				var texture = textureAtlas.getTexture(animationObjectConfig1.get_regionID());
				if(js_Boot.__instanceof(texture,com_github_haxePixiGAF_display_GAFScale9Texture) && !animationObjectConfig1.get_mask()) {
				...
				break;
			case "timeline":
				var timeline = gafAsset.getGAFTimelineByID(animationObjectConfig1.get_regionID());
				displayObject = new com_github_haxePixiGAF_display_GAFMovieClip(timeline,this.get_fps() | 0,false);
				break;
			}
			if(animationObjectConfig1.get_maxSize() != null && js_Boot.__instanceof(displayObject,com_github_haxePixiGAF_display_IMaxSize)) {
				var maxSize = new PIXI.Point(animationObjectConfig1.get_maxSize().x * this._scale,animationObjectConfig1.get_maxSize().y * this._scale);
				(js_Boot.__cast(displayObject , com_github_haxePixiGAF_display_IMaxSize)).set_maxSize(maxSize);
			}
			this.addDisplayObject(animationObjectConfig1.get_instanceID(),displayObject);
			if(animationObjectConfig1.get_mask()) {
				this.addDisplayObject(animationObjectConfig1.get_instanceID(),displayObject,true);
			}
			if(this._config.get_namedParts() != null) {
				var this1 = this._config.get_namedParts();
				var key = animationObjectConfig1.get_instanceID();
				var _this = this1;
				var instanceName = __map_reserved[key] != null ? _this.getReserved(key) : _this.h[key];
				if(instanceName != null) {
					if(!Object.prototype.hasOwnProperty.call(this,instanceName)) {
						var this2 = this._config.get_namedParts();
						var key1 = animationObjectConfig1.get_instanceID();
						var _this1 = this2;
						this[__map_reserved[key1] != null ? _this1.getReserved(key1) : _this1.h[key1]] = displayObject;
					}
	...
	,updateBounds: function(bounds) {
		haxe_Log.trace("TODO updateBounds",{ fileName : "GAFMovieClip.hx", lineNumber : 1037, className : "com.github.haxePixiGAF.display.GAFMovieClip", methodName : "updateBounds"});
	}
	,__debugHighlight: function() {
		if(isNaN(this.__debugOriginalAlpha)) {
			this.__debugOriginalAlpha = this.alpha;
	...
	,removeChildAt: function(index) {
		haxe_Log.trace("TODO: removeChildAt",{ fileName : "GAFMovieClip.hx", lineNumber : 1107, className : "com.github.haxePixiGAF.display.GAFMovieClip", methodName : "removeChildAt"});
	...
	,destroy: function(options) {
		...
		var _this = this._stencilMasksDictionary;
		var lMask = new haxe_ds__$StringMap_StringMapIterator(_this,_this.arrayKeys());
		while(lMask.hasNext()) {
			var lMask1 = lMask.next();
			lMask1.destroy();
		}
	...
	,changeCurrentFrame: function(isSkipping) {
		...
		if(this._nextFrame >= this._startFrame && this._nextFrame <= this._finalFrame) {
			...
			this._lastFrameTime += this._frameDuration;
			var resetInvisibleChildren = true;
		}
		this.runActions();
		if(this._disposed) {
			return;
...
var com_github_haxePixiGAF_display_GAFTextField = $hx_exports["GAF"]["GAFTextField"] = function(config,scale,csf,debug) {
	...
	if(config.get_textFormat().align == "center") {
		...
		var tmp = config.get_width();
		this.textField.x = tmp / 2;
...
com_github_haxePixiGAF_utils_DebugUtility.getColor = function(difficulty) {
	...
	var color = (difficulty >> 1) + 127 << 24 | colorArr[0] << 16 | colorArr[1] << 8;
	return color;
};
...
com_github_haxePixiGAF_utils_RectangleUtility.union = function(pA,pB) {
	...
	var lRight = Math.max(pA.x + pA.width,pB.x + pB.width);
	var lBottom = Math.max(pA.y + pA.height,pB.y + pB.height);
	return new PIXI.Rectangle(lX,lY,lRight - lX,lBottom - lY);
};
...
com_github_mathieuanthoine_gaf_Main.prototype = {
	...
	,initGAFMovieClip: function(swfName) {
		...
		var timeline = this.gafBundle.getGAFTimeline(swfName,"rootTimeline");
		this.gafMovieClip = new com_github_haxePixiGAF_display_GAFMovieClip(timeline);
